...
2. Inspect your own reasoning/thoughts (thinking log) during task execution. Your goal is to understand exactly how the current phrasing of the instructions led the agent to an undesirable result.
...
4. **Proposals for Instruction Correction:** Provide specific formulations to correct the problematic instructions (translating abstract rules into clear steps, adding new negative prompts, etc.). IT IS FORBIDDEN to make modifications independently — only propose them to the operator as text insertions (code snippets or diff blocks).

## Strict Constraints
- IT IS FORBIDDEN to independently make any modifications to project files or perform code corrections during the debugging process. Only reading, analysis, and generating a text report with justifications and a plan are permitted.
...
- Any claim of violation must be accompanied by a link to a specific rule described in the official instructions (for example, rules of style, execution context, or workspace constraints).
