...
3. If splitting is justified, extract stages into separate instructions following the file model from @draft(workspace). Organize them so the operator can pass each subsequent stage only after the previous one has been fully executed in an isolated context.
...
6. Compare the result against the original instructions. Complete refactoring only after verifying that every original meaning is preserved, the new structure conforms to @draft(workspace), and the file separation serves context isolation.
