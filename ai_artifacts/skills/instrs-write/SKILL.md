...
* **Предотвращение загрязнения контекста**: Не дробите задачи механически. Выделяйте задачи в отдельные файлы инструкций только в том случае, если их детальные описания конфликтуют и вызывают загрязнение контекста (например, глубокий ресёрч и сложный код).
* **Изолированное выполнение**: Формируйте эти отдельные инструкции так, чтобы оператор мог передавать их агенту последовательно, позволяя агенту выполнять каждый шаг в чистом, сфокусированном контексте.
